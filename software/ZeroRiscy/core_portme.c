...
#endif
  volatile ee_s32 seed4_volatile=1;//ITERATIONS;
...
/*void send_gpio(unsigned short value){
    unsigned int addr = 0x20002000;
    unsigned short reset = 0;
    asm volatile ("sh %0,0(%1)\nsh %2,0(%1)"::"r"(value), "b"(addr), "b"(reset));
}

void send_gpio_int(unsigned int value){
    unsigned int addr = 0x20002000;
    unsigned short a = 512 | (value & 0xFF);
    unsigned short b = 512 | ((value & 0xFF00) >> 8);
    unsigned short c = 512 | ((value & 0xFF0000) >> 16);
    unsigned short d = 512 | (value >> 24);
    unsigned short r = 0;
    asm volatile ("sh %0,0(%1) \
                    \nsh %2,0(%1)\
                    \nsh %3,0(%1)\
                    \nsh %4,0(%1)\
                    \nsh %5,0(%1)"::"r"(a), "b"(addr), "b"(b), "b"(c), "b"(d), "b"(r));
}*/
